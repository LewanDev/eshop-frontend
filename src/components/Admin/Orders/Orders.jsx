import React, { useEffect, useState } from "react";
import Navbar from "../../NavBar/Navbar";
import BackButton from "../../Misc/BackButton";
import Footer from "../../Footer/Footer";
import formatPrice from "../../../utils/formatPrice";

const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5000/api";

const downloadXLS = async (orderId) => {
  try {
    const res = await fetch(`${API_URL}/order/${orderId}/xls`);

    if (!res.ok) throw new Error("No se pudo descargar el archivo");

    const blob = await res.blob();
    const url = window.URL.createObjectURL(blob);

    const a = document.createElement("a");
    a.href = url;
    a.download = `Pedido_${orderId}.xlsx`;
    a.click();
    a.remove();

    window.URL.revokeObjectURL(url);
  } catch (err) {
    console.error("❌ Error al descargar XLS:", err);
    alert("Error descargando archivo.");
  }
};

const Orders = () => {
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");

  // 🔥 Traer pedidos al cargar la página
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await fetch(`${API_URL}/order`);
        const data = await res.json();

        if (data.success) {
          setOrders(data.orders);
        }
      } catch (err) {
        console.error("Error al traer pedidos:", err);
      } finally {
        setLoading(false);
      }
    };

    fetchOrders();
  }, []);

  // 🔍 FILTRO DE BÚSQUEDA
  const filteredOrders = orders.filter((ord) => {
    const search = searchTerm.toLowerCase();

    const numberMatch = ord.orderNumber?.toString().includes(search);
    const nameMatch = ord.cliente?.name?.toLowerCase().includes(search);

    return numberMatch || nameMatch;
  });

  return (
    <div className="flex flex-col min-h-screen">
      <Navbar />
      <main className="flex-1 p-6 bg-(--color-lighter)">
        {/* 🔙 Back + Título centrado */}
        <div className="relative flex items-center justify-center my-4">
          <div className="absolute left-0">
            <BackButton />
          </div>
          <h2 className="text-2xl font-bold">Listado de pedidos</h2>
        </div>

        {/* 🔍 Campo de búsqueda */}
        <div className="my-4 flex justify-center">
          <input
            type="text"
            placeholder="Buscar por cliente o número de pedido..."
            className="border p-2 rounded w-full mb-4 bg-(--color-lightest)"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>

        {/* Tabla de pedidos */}
        {loading ? (
          <p>Cargando pedidos...</p>
        ) : filteredOrders.length === 0 ? (
          <tr>
            <td colSpan="5" className="text-center py-4 text-(--color-gray)">
              No hay pedidos para mostrar
            </td>
          </tr>
        ) : (
          <table className="w-full bg-(--color-lightest) shadow rounded">
            <thead>
              <tr className="bg-(--color-gray) text-(--color-lighter) text-lg">
                <th className="p-2"># Pedido</th>
                <th className="p-2">Cliente</th>
                <th className="p-2">Artículos</th>
                <th className="p-2">Total</th>
                <th className="p-2">Fecha</th>
                <th className="p-2"></th>
              </tr>
            </thead>

            <tbody>
              {filteredOrders.map((ord) => (
                <tr key={ord._id} className="border-t text-center text-lg">
                  <td className="p-2 font-bold">{ord.orderNumber}</td>
                  <td className="p-2">{ord.cliente?.name || "Sin nombre"}</td>
                  <td className="p-2">
                    {ord.items.map((i) => (
                      <div key={i._id}>
                        {i.itemId?.name || "Item"} x{i.cantidad} ({i.color})
                      </div>
                    ))}
                  </td>
                  <td className="p-2">{formatPrice(ord.total)}</td>
                  <td className="p-2">{ord.fecha}</td>
                  <td className="p-2">
                    <button
                      className="btn-green-icon"
                      onClick={() => downloadXLS(ord._id)}
                    >
                      <svg
                        width="32px"
                        height="32px"
                        viewBox="0 0 512 512"
                        version="1.1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink"
                        fill="#000000"
                      >
                        {" "}
                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>{" "}
                        <g
                          id="SVGRepo_tracerCarrier"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        ></g>{" "}
                        <g id="SVGRepo_iconCarrier">
                          {" "}
                          <title>xls-document</title>{" "}
                          <g
                            id="Page-1"
                            stroke="none"
                            stroke-width="1"
                            fill="none"
                            fill-rule="evenodd"
                          >
                            {" "}
                            <g
                              id="icon"
                              fill="#ffffff"
                              transform="translate(83.411200, 42.666667)"
                            >
                              {" "}
                              <path
                                d="M236.5888,7.10542736e-15 L1.92213333,7.10542736e-15 L1.92213333,234.666667 L44.5888,234.666667 L44.5888,192 L44.5888,169.6 L44.5888,42.6666667 L218.882133,42.6666667 L300.5888,124.373333 L300.5888,169.6 L300.5888,192 L300.5888,234.666667 L343.255467,234.666667 L343.255467,106.666667 L236.5888,7.10542736e-15 L236.5888,7.10542736e-15 Z M338.244267,341.851733 C335.534933,339.803733 332.6336,338.011733 329.582933,336.475733 C326.4896,334.939733 320.2176,332.209067 310.766933,328.283733 C301.252267,324.379733 295.214933,321.435733 292.718933,319.473067 C290.222933,317.5104 288.964267,314.801067 288.964267,311.3664 C288.964267,307.3984 290.4576,304.113067 293.4016,301.5104 C296.2176,299.121067 300.3776,297.905067 305.796267,297.905067 C310.5536,297.905067 315.118933,298.417067 319.492267,299.441067 C323.844267,300.4864 330.3936,302.577067 339.054933,305.777067 L344.8576,282.267733 C330.1376,277.617067 316.5056,275.313067 303.982933,275.313067 C286.6176,275.313067 274.1376,280.6464 266.564267,291.313067 C261.5936,298.1824 259.140267,306.161067 259.140267,315.2704 C259.140267,325.809067 262.894933,333.8944 270.4256,339.505067 C273.070933,341.489067 275.9936,343.259733 279.150933,344.8384 C282.3296,346.395733 287.7696,348.785067 295.470933,351.985067 C305.902933,356.273067 312.452267,359.409067 315.0976,361.4784 C317.742933,363.5264 319.086933,366.385067 319.086933,370.0544 C319.086933,374.171733 317.7216,377.627733 315.012267,380.465067 C311.876267,383.707733 306.990933,385.329067 300.3776,385.329067 C290.180267,385.329067 277.764267,382.2784 263.1296,376.113067 L258.948267,400.7104 C272.644267,405.531733 286.958933,407.9424 301.892267,407.9424 C316.2496,407.9424 327.3856,404.955733 335.342933,399.003733 C344.708267,391.9424 349.358933,381.531733 349.358933,367.8144 C349.358933,356.1664 345.668267,347.5264 338.244267,341.851733 M95.936,277.568 L65.728,319.338667 L35.904,277.568 L2.34666667,277.568 L47.3813333,339.946667 L-1.42108547e-14,405.696 L34.0693333,405.696 L64.2986667,362.922667 L94.6773333,405.696 L129.472,405.696 L82.3893333,340.672 L128.938667,277.568 L95.936,277.568 Z M154.2848,277.568 L154.2848,405.696 L238.1888,405.696 L238.1888,383.082667 L183.1488,383.082667 L183.1488,277.568 L154.2848,277.568 Z"
                                id="XLS"
                              >
                                {" "}
                              </path>{" "}
                            </g>{" "}
                          </g>{" "}
                        </g>{" "}
                      </svg>
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </main>
      <Footer />
    </div>
  );
};

export default Orders;
